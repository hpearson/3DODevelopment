...
void updateCelData(mesh *ms)
{
	// if we change quad fadePals on the fly (for lighting)
	// we should make palette object and add palshadesNum
...
void renderTransformedGeometry(Item screen, mesh *ms)
{
    ...
    const int xMin = 64;
    const int xMax = SCREEN_WIDTH - 65;
    const int yMin = 64;
    const int yMax = SCREEN_HEIGHT - 65;
